...
import javax.swing.JOptionPane;
    public class Lab06
   {
      ...
      {
    	 String[] mapOption = {"tasks1","tasks2","tasks3"};
         String filename = (String)JOptionPane.showInputDialog(null,"What robot world?",null,JOptionPane.QUESTION_MESSAGE,null,mapOption,"tasks1");
      ...
      { //go to the wall, pick up all the beepers (max one per pile)
         ...
         while(true) {
        	 if(temp.nextToABeeper() & temp.frontIsClear()) {
        		 ...
        		 ++beeperNum;
        		 temp.move();
        	 }else if(temp.nextToABeeper() & !temp.frontIsClear()) {
        		 temp.pickBeeper();
        		 ++beeperNum;
        	 }else if(!temp.nextToABeeper() & temp.frontIsClear()){
        		 temp.move();
         ...
         System.out.println("total amount of beepers in task04 is "+beeperNum);
      }
      ...
      { //go to the wall, pick up all the beepers
         ...
         int beeperNum = 0;
         while(true) {
        	 if((temp.frontIsClear() & temp.nextToABeeper()) | (!temp.frontIsClear() & temp.nextToABeeper())) {
            	 temp.pickBeeper();
            	 ++beeperNum;
             }else if(temp.frontIsClear() & !temp.nextToABeeper()) {
            	 temp.move();
             }else {break;}
         }
         System.out.println("total amount of beepers in task05 is "+beeperNum);
      }
      ...
      { //go to the end of the row of beepers, there is one gap
         ...
         int gapNum = 1;
         while(gapNum >= 0 | temp.nextToABeeper()) {
        	 temp.move();
